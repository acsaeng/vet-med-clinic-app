...
public class PhotosController {
	 ...
	 */
	@GetMapping(path="{id}") 
	public Photo selectPhotoById(@PathVariable("id") String idStr) {
		try {
			UUID id = UUID.fromString(idStr);

			// Throw exception if UUID is valid but does not exist in database
			return photosService.selectPhotosById(id).orElseThrow(ApiExceptions.invalidIdException());
	...
	public void deletePhotoById(@PathVariable("id") String idStr) {
		try {
			UUID id = UUID.fromString(idStr);
	 ...
	 */
	@PutMapping(path = "{id}")
	public void updatePhotoById(@PathVariable("id") String idStr, @RequestBody Photo photoToUpdate) {
		try {
			UUID id = UUID.fromString(idStr);
